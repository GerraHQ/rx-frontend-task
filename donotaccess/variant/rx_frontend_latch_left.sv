// rx_frontend — CALIBRATION VARIANT (answer key; never shipped). Functionally correct
// integration (backpressure off the watermark + status wired right), but the fill encode
// is left as an `always @*` with an incomplete case, so it synthesizes with an inferred
// latch and fails the latch-free synthesis + lint layers: functional 6/6, reward ~0.70.
module rx_frontend #(
    ...
    assign s_ready_o = in_can_load && !almost_full_o;   // correct: off the watermark
    ...
    status_csr #(.n_p(2)) sts (
        ...
        .event_i({almost_full_o, overflow_event}), .sticky_o(status_o)   // correct
    );
    assign err_o = status_o[0];

    // fill encode left with an inferred latch (incomplete case, no default)
    always @* begin
