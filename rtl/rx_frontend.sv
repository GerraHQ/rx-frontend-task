// rx_frontend — link ingress front-end. Composes two provided library blocks (in
// rtl/lib/, do not edit them):
//   stage 1: a registered input beat (one cycle of in-flight latency)
//   stage 2: fifo_v3    — vendored PULP elastic FIFO (full / empty / usage interface)
//   stage 3: status_csr — generic sticky cross-block status
// The integration on top is the deliverable: the watermark (almost_full) and its early
// backpressure, sticky overflow, cross-block status aggregation, the true occupancy
// count (fifo_v3's usage_o truncates at full), and a latch-free fill encode.
//
// Bring-up status: not signed off — fails under sustained upstream load and on the
// status / level outputs.
module rx_frontend #(
    ...
    // Backpressure upstream while the store cannot take another beat.
    assign s_ready_o = in_can_load && !fifo_full;
    ...
    status_csr #(.n_p(2)) sts (
        ...
        .reset_i,
        .clear_i  (sts_clear_i),
        .event_i  ({1'b0, overflow_event}),
        .sticky_o (status_o)
    );
    assign err_o = status_o[0];

    // ---- top-level coarse fill encode ----
    always @* begin
